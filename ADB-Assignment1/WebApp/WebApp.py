...
def search_by_everything(name,min_sal,max_sal,room,telnum):
    try:

        conn = pyodbc.connect(connstr)
        cursor = conn.cursor()
        #print(min_sal,"min_sal")
        search_query = "SELECT * FROM av_people WHERE 1=1 "
...
def home_function():
    if request.method == 'POST':
        ...
        if 'filter_name' in request.form :
            ...
            min_sal = request.form["filter_min_sal"]
            max_sal = request.form["filter_max_sal"]
            room = request.form["filter_room"]
            telnum = request.form["filter_telnum"]
            filtered_Info = search_by_everything(name, min_sal, max_sal, room, telnum)
            t_headings = ["Name", "State", "Salary", "Grade", "Room", "Telnum" ,"Picture", "Keywords", "ImageURL"]

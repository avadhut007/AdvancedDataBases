...
def home_function():
    if request.method == 'POST':
        try:
            if 'freq_count' in request.form:
                search_text = request.form["search_text"]
                freq_count = int(request.form["freq_count"])
                print(search_text)
                return render_template('home.html')

            if 'len_count' in request.form:
            ...
            if 'search_mag' in request.form:
                ...
                search_query = "SELECT * FROM av_asg7 WHERE 1 = 1 "
                if len(magnitude)!=0:
            ...
            if 'group_from_date' in request.form:
                ...
                for i in range(1,7):
                    search_query = "SELECT * FROM av_asg7 WHERE mag BETWEEN " + str(float(i)) + " AND " + str(float(i+1)) + " AND time between '" + str(group_from_date) + "' AND '" + str(group_to_date) + "'"
            ...
            if 'no_of_quakes' in request.form:
                ...
                search_query = "SELECT TOP " +no_of_quakes+ " * FROM av_asg7 ORDER BY mag DESC"
